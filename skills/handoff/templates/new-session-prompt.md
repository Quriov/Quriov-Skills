...
variables:
  ...
  - "{{worktree_reclaim}}": Step 4c 写的前任 worktree 报告段 —— 只报路径与住户, ⛔ 不含删除指令 (没有前任 worktree 时整节删掉)
---
...
(用户 2026-09-09 拍:「基于一个过期的 WorkTree 的情况给出的判断, 那肯定也是过期的, 这个影响很大」)
...
> ⛔ **别把这一段留空** —— 查到什么写什么(例:`⚠ 前任 worktree <路径> 有未提交改动 —— 需要人看一眼是否还要`)。
> **空着和"没有前任 worktree"长得一样。**
>
> 🚨 **handoff 不负责回收(2026-08-31 用户拍板)** —— 回收归 harness 的池化机制。这一段只报「你住在哪、旁边是谁」, ⛔ **不含任何删除指令**。
> (本模板 08-31 后漏改, 一直写着「两步都过了才 git worktree remove」; 2026-09-14 G 线 v2.7 实撞后订正。)
>
> **`{{worktree_reclaim}}` 至少要展开成这四样(一样都不能省)**:
> ```
> 📍 绝对路径 + 分支 + 查询时刻
...
> ⛔⛔ 两步都过了也【不要删】—— 只把结论(路径 + 住户 + 查询时刻)如实写给用户
> ```
