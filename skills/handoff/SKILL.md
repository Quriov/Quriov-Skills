...
- **🤝 协作方清单 lint (条件必填, 而且【条件本身必须是数出来的】)**:
  ...
  **阳性对照的真值也是人填的, 它一样会错。**
  ...
  📌 **实测(iOS v5.0 报回, 2026-09-03, 它自己当场撞到并回滚)**:
...
- **⭐ 交接期间接活 —— 拆成【一道真闸】+【一条老实的提醒】, 别混成一句**:
  ...
  **这是它诚实的边界, 但读的人容易把 PASS 读成「我刷对了」。**
  ...
  ⭐⭐ **而「提醒」内部还有一档差别, 值得单写(AI Harness 线 2026-09-04 给的, 采纳)**:
...
- ⭐ **席位表自维护(项目有席位注册表时必做;没有则整段跳过)** —— 2026-09-07 加,挂点由使用方定在这里:
  ...
  📌 出处: 眼镜仓 `context/methods/session-registry.md` §维护约定第 1 条(宇通 0907 拍「每席自己维护自己那行」; 那份表 0907 核出 3 席漂了没人知道 —— **「记得改」已被证明不成立, 所以挂在必经动作上**)。
  ⚠ 依据层级(按本 skill 自己的判据标出处): 那条约定的原文我读过(PR #4352), 宇通原话是总控线转述, 未一手核。
...
- ⭐⭐ **改动对账 lint (比产物表【更早一层】—— 产物表问"文档里有没有这几段", 这条问"这一轮我到底动过什么")**:
  ...
  **要等就等在已提交的分支上。**
  ...
  ⚠⚠ **第二种丢法与共享无关: 改动放在【会被整个清掉的目录】里**(CI/AI Review v6.8, 2026-09-14 实撞)。
