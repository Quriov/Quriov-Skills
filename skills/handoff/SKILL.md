...
- ⚠ **磁盘已经是最新时, 这一步本来就不必跑** —— 先 `grep` 一下磁盘 rev, 已经等于源仓就跳过。`update -g` 会更新本机**所有**全局 skill, 作用面比这件事需要的大得多。
...
| **记录**(写进交接文档) | **本 session 所有有过往来的**, 每条附一句「结没结 / 结论是什么」 | 零成本, 几行字; 让接班知道谁是谁、找得到人 |
...
⇒ **先列一次当前还活着的 session / 联系人**(你的环境提供什么就用什么), 按**线名**而不是按 id 匹配, 再发。
...
- **🤝 协作方清单 lint (条件必填)**: **本 session 有过跨方往来** → doc 里必须有 §🤝 小节, 且每条注明「结没结」;
  缺 → 补齐再 output。**没有任何往来 → 整条跳过**(0 个协作方是正常结果, 不是漏项)。
- **⭐ 交接期间接活 lint (防上下文丢失)**: 进入交接状态(Step 0.6)之后**当场处理过的"现在"类消息**,
