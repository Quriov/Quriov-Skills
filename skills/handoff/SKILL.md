...
| **CI 检查显示"全绿、已跑完"** | 本次推送真跑过且通过 | ⭐ **PR 与主干冲突 ⇒ 本次零 run, 你看到的是【上一次推送】的结果** |
...
**判别式(两句, 都要)**:
1. 查 PR 的**可合并状态**而不是检查状态 —— 冲突态要单独识别出来;
2. **返回"未知"不是"没冲突", 是"还没算完"** ⇒ ⚠ **必须重查到「未知」归零为止, 不是"再查一次"**
   (实测第 2 次仍可能剩 1 个, **第 3 次才收敛**)。
