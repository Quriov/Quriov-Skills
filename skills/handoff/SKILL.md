...
> 🚨 **第 0 步是后加的, 因为漏了它会死循环** —— 📌 实测(控制变量, 两条线各自跑):
> ```
...
> ```bash
> # ⚠ 必须在目标目录【之外】跑(理由见下方"查占用会自己制造占用")
> lsof -a -d cwd -c claude 2>/dev/null | grep -F "<绝对路径>"   # 必须为空
> ```
> **双向验过(2026-08-29)**: 拿一个**确实有人住**的 worktree 跑 → 打印出 2 个 `claude` 进程(**咬得住**);
> 拿本次真要回收的那个跑 → **空**(不误报)。
> ⚠ **它只看得见 `claude` 进程** —— 有人把编辑器 / shell 停在那儿它查不到。**这是已知盲区, 不是"通过了"。**
> 🔑 **git 三条判据问的是【仓库状态】, 而这个障碍在【进程状态】—— 两者没有交集**,
> 所以它不是"第四条同类判据", 是**另一个维度**。
> 📌 **本条来自一次真实漏判**: 某线接班时交接单说前任 worktree「可回收」并列了三条 git 判据, **三条全部成立**,
> 而那个目录里住着**前任自己**(活了 16 小时的进程)。
...
♻️ 前任 worktree 可回收: <绝对路径>(分支 <branch>,已确认工作区干净、无未推送 commit)
   ...
   自查通过后,再确认没有别人住在里面(⚠ 在该目录【之外】跑,否则这条检查会自己制造占用):
      lsof -a -d cwd -c claude 2>/dev/null | grep -F "<绝对路径>"   # 必须为空
      有输出 ⇒ 停,别删 —— 有 session 正住在里面(可能是前任还没退,也可能是别条线)。
      ⚠ 它只看得见 claude 进程,查不到停在那儿的编辑器/shell —— 这是已知盲区,不是"通过了"。

   都过了:git worktree remove <绝对路径>
