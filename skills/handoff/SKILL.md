...
2. ⭐ **先 `git fetch origin`**, 再 `git rev-list --count HEAD..origin/main` → **你手上这份落后主干多少**。不是 0 就**先拉平再动手** — 怎么拉平见下方 🚨
   ⚠ **2026-09-09 补 fetch**:此前这条比的是本地缓存的 origin/main, 缓存多久没刷、数出来的 0 就多假。同日用户拍:接班模板 §1 也加了同一套「fetch → 量 → 拉平」(此前只有出棒方量, 接班方不量 ⇒ 轮换来的陈旧 worktree 没有任何一步会拉平, 实测两条活线落后 346 / 483)。
...
> 📊 **实测 (2026-08-25, 256 次真实 handoff 的成本分析, 由 F 线提供)**: 昂贵档 **84%** 的 Bash 调用是彼此独立的**只读状态查询**, 被一轮一条地跑 —— 合并它们是这类 handoff 成本差距里**最大的一块**(重算后解释度 **76%**)。
...
> 📌 实测(D 线 v6.1, 2026-09-03):它照自然写法用 `grep -c '^### [0-9]'` 数信号条目, 得 **29**,
...
>
> 🔑 **为什么第一种自动、后两种问** (2026-08-21 用户拍): 第一种**答案唯一** —— 在已合并的分支上继续 commit 是本 skill 明令禁止的 (anti-pattern #8), 没有第二个选项可选, 问一次纯消耗用户注意力 (仪式花的是他的注意力)。后两种**答案不唯一** (脏工作区那些改动要不要带走 / 这条分支是不是其实就该继续用), 必须人判。
>
...
> 2026-09-03 用户拍板「账本要边做边追加」(挂在两个必经动作上)。**A 线当轮去执行时发现无处可落** ——
...
📌 **实测 (2026-08-28, 两条线各撞一次)**: 某线的**显示标题**是「A 线 v4.1」, 而它的**投递地址**
叫 `track-g-v2-2-147fa1-78`(worktree 派生名) —— **两者交叉错位到毫无关联**。
...
3. **📋 What shipped this turn** — ⭐ **按下面三档筛, ⛔ 别一股脑全列**
   ...
   > `#1234` 卡顶过期两周**骗了两棒人各查一遍** · 某线卡里那整段「✅ 已排除(有据, 别重查)」·
...
>    📌 **实测**: 我按 fallback 去量 C 仓的项目级模板, 量到「缺四段」, 并据此向用户建议**删掉它**;
>    用户批准后我临删前查了一眼, 发现那个检出挂在一条**从未推送、落后 63 个 commit、停在 8/27 的本地分支**上。
...
> **漏了三处, 而三处都还标着 ✅**: 本文件**上面那个「原样写进接班 prompt」的 fenced block**、
> `templates/new-session-prompt.md`、以及一张**自动注入的 memory 卡**(它的 `description` 已订正、
> 而正文的 `How to apply` 步骤没有)。
> 🔑 **漏掉的那处恰恰是会被【照抄】的那处** —— 规则文件是给人读的, 模板是**被复制进下一棒脑子里的**。
...
>   却有三处仍写着「那个工具**不认**另一种地址 / 两套地址系统不通」, 而三方实测**两种它都收**。
...
>   对方改完之后回我一条**对我不利的**: 它把那个工具的 **schema 调出来读了** —— 文档里
...
> | 扫法 | 结果 |
> |---|---|
> | 早先的**全称量词闸**(任何量词都要出处) | 6 块板 **55 句全中, 几乎全是正当用法** ⇒ 已判定不该做 |
...
- **🤝 协作方清单 lint (条件必填, 而且【条件本身必须是数出来的】)**:
  ...
  **阳性对照的真值也是人填的, 它一样会错。**
  ...
  📌 **实测(A 线 v5.0 报回, 2026-09-03, 它自己当场撞到并回滚)**:
  它在一道闸上修假阳性, **改完假阳性 5→0、其余对象零变化 —— 看起来完美**。
  然后它**造了一个必须报警的样本**去验:**它不报了**;拿**改前**的版本跑同一个样本, **会报**。
  ⇒ **不是修好了, 是把整道闸弄哑了。** 已逐字节回滚。
...
- **⭐ 交接期间接活 —— 拆成【一道真闸】+【一条老实的提醒】, 别混成一句**:
  ...
  **把判据写在【会被诱惑的那一刻】能看到的地方 —— 那种提醒比写进文档的提醒强一个量级。**
  📌 **活例**: 它今天补规则时撞上自己设的体量闸(只剩 23 字节)。
  **它完全可以改一行常量让闸变绿** —— 而闸里写死着「⛔ 别为了让闸变绿调高这个数」。
  它照判据把叙述搬进 `reference/`, 降到 19,668, **没有调上限**。
...
- ⭐ **席位表自维护(项目有席位注册表时必做;没有则整段跳过)** —— 2026-09-07 加,挂点由使用方定在这里:
  ...
  🔑 为什么挂在收尾而不是开局: 收尾时你最清楚这一棒实际管了什么; 开局时你只知道交接单写了什么。
  📌 出处: 某仓 `context/methods/session-registry.md` §维护约定第 1 条(用户 0907 拍「每席自己维护自己那行」; 那份表 0907 核出 3 席漂了没人知道 —— **「记得改」已被证明不成立, 所以挂在必经动作上**)。
  ⚠ 依据层级(按本 skill 自己的判据标出处): 那条约定的原文我读过(PR #1205), 用户原话是 D 线转述, 未一手核。
...
- ⭐⭐ **改动对账 lint (比产物表【更早一层】—— 产物表问"文档里有没有这几段", 这条问"这一轮我到底动过什么")**:
  ...
  **要等就等在已提交的分支上。**
  ...
  交接用的临时树建在 session 的 scratchpad 里, 攒了一晚没提交的交接改动(账本几条 · 交接单十几处 · 接班 prompt · 检查点指针);
  撞账号限额停了一夜, 会话重启后 scratchpad 被清空 —— 树连同改动一起没了, 只能照对话原文重放。
...
> (本段 08-31 后漏改, 一直写着「三条判据已过, 可回收」与「唯一出口」; 2026-09-14 G 线 v2.7 跑 handoff 时连同两份模板一起撞出。)
