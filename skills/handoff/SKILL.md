...
- **🤝 协作方清单 lint (条件必填, 而且【条件本身必须是数出来的】)**:
  ...
  ✅ **数出来**(用你环境里有的东西; 下面是一种可行口径):
    · **收到的**跨方消息 —— 在本 session 的对话记录里按**结构标记**数(跨 session 消息通常带包裹标记),
      **按结构数, 不按语义猜**: 真·用户发言没有那个标记, 所以"把用户误当协作方"结构上就不会发生
  ...
  **那个 0 必须是量出来的, 不是假设出来的。**
  ...
  > ⇒ **给"找到自己的记录"这一步加一条自证**(行数阈值 / 按本 session 独有的内容定位), **别信"最近修改的那个"**。
