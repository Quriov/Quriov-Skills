...
```bash
grep handoff-skill-rev <本 skill 的 SKILL.md>   # 已经等于源仓 ⇒ 这一步整段跳过
```
...
- **🤝 协作方清单 lint (条件必填, 而且【条件本身必须是数出来的】)**:
  ...
  ✅ **数出来** —— ⚠ **下面这个口径是被独立验过的, 请照抄; 它的两个"显然写法"两个方向都会错**:
  ...
  ✅ **三条缺一不可**:
    ...
    · **发出的**跨方消息 —— 扫工具调用记录里的发送类调用

  📌 **双向对照 (报回者跑过, 两头都过)**: 阳性 = 一条已知真值的记录 → 正确数出 2 个协作方 / 发出 1 条;
  阴性 = 一条 7864 行、确实没有跨方往来的记录 → `0` / `0` —— **是真的 0, 不是坏掉的 0。**
