...
> 🔑 **为什么这行值得写**: 它是版本号的**可 grep、带历史、无副作用**的备份载体。
> (真正的主路径是 Step 4b 的**两步读法** —— 那条不依赖任何人写过任何东西。这行是冗余的第二条腿。)
> 历史背景: 版本号曾经**唯一的载体是桌面版 UI 里那行标题字符串**, 而当时以为 AI 读不到它 ⇒ 用户每次
> 手动改名都只是给链条续一口气, 那口气没被记进任何下一棒读得到的地方 ⇒ 下一棒又回到零。
>
...
2. **当前版本从哪来** —— ⚠ **不是「你自己知道」。你读不到自己 session 的标题**:
   ...
   1. ⭐ **两步读法 —— 直接问出你自己的标题**(下方详解)。**主路径, 因为它不依赖任何人做过任何事。**
   2. 最近一份 handoff 文档顶部的 `Session:` 行 (Step 2c) —— 冗余备份, 可 grep、带历史、无副作用
   3. 项目 active-tracks / 最近 handoff 正文里的标题行
   4. 仍然拿不到(例如这条线**从没被命名过**) → **问用户, 别猜** —— 猜错会让版本号断档或倒退

   #### ⭐ 两步读法 (2026-08-25 实测通过, 净效果为零)

   `set_session_title` 的返回值**会带上旧标题**。改一次再改回去, 就读到了:

   ```
   set_session_title(session_id="self", title="__probe__")
       → Renamed this session to "__probe__" (was "<你的真实标题>")     ← 拿到了

   set_session_title(session_id="self", title="<你的真实标题>")
       → Renamed this session to "<真实标题>" (was "__probe__")          ← 改回, 净变化为零
   ```

   ⚠ **两步必须连着做, 中间不要插入别的工具调用** —— 万一第二步没跑成, session 会停在
   `__probe__` 上。正因如此**临时值要用一眼看得出是异常的**(别用看着像正经标题的字符串)。

   > 🔑 **为什么这条能当主路径, 而文档字段只能当备份**:
   > 文档字段要求**上一棒写过**, 两步读法**什么都不要求** —— 任何一棒、任何时候都能自己读到自己是谁。
   > ⇒ 版本号链条因此从「每一环都是单点、漏一棒就回到零」变成「**每棒独立自足**」:
   > 人只需要在这条线**诞生时命名一次**(谁叫什么名字只能人定), 之后**永远不会再断**。
   > 两条一起留着是**故意的冗余**: 文档字段能被 grep、留得下历史、且不需要副作用调用。
