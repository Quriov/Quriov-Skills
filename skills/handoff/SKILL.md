...
📊 **实测代价 (2026-08-25, 256 次真实 handoff 的成本分解, 由 Harness 线提供)**: **最贵那 25%** 每次 handoff 发 **22.1 条**零工具调用的纯文字轮(中位 104 字, 83% 不到 300 字)。
**22 条 × 进入时 67 万上下文 ≈ 14.7M, 占那一档总成本 55.6M 的 26%** —— 写约 2,300 字, 花掉四分之一的钱。
因为每一条纯旁白轮, 都要**把当时的全部上下文重读一遍**才能说出那 104 个字。

⚠ **22.1 是最贵那档的均值, 不是典型值** (2026-08-25 由 Harness 线补正, 出处同上): 全体普通业务类
**175 次**的真实分布是 p25=**6** / 中位=**12** / p75=**19** / 最大 43 / sd 8.5 —— 而且 **33% 的旧版
handoff 本来就 ≤8 条**。
⇒ **别把「从 22 压到 5」当目标**: 那是拿最贵档当基线, **参照系错了**; 三分之一的旧版本来就做得到 5-8 条,
所以"某次只发了 5 条"**根本不证明规矩起了作用**。判据见下方 📌。

⚠ **这不是让你少说话。用户要能跟上进展 —— 文字量不变, 只是别让它单独占一轮。**

✅ **三种纯文字轮是合理的, 别为了合并而消灭它们**:
1. **要问用户**(等回答, 本来就没工具可调)
2. **活干完了汇报结果**
3. **本 skill 明文要求的独立发声**(Step 0.5 skill 更新了要告知一行 / 任务板探测三条全落空必须出声 /
   Step 2b 自动切了分支要告知)

⇒ 合理下限**不是 0, 也不是任何一个固定数字** —— 是上面这三类**在那次 handoff 里实际发生了几次**。

> 📌 **判据是合规, 不是统计** (2026-08-25 定, 取代原先的"压到 5"): 数一次 handoff 里
> 「**没挂任何工具调用的纯文字轮**」有几条, 逐条对照上面三类看解释不解释得通 —— **解释不通的那些,
> 就是该合并而没合并的。**
>
> **这个判据 N=1 就有信息量**, 而统计对比不行:
> - 方差太大(sd 8.5, 范围 1–43) ⇒ 要在噪声里看出中位数从 12 降到 5, 需要 10–15 个同规模样本
> - 旁白占比**随 handoff 规模变**, 不是常数: 小 handoff(≤30 轮)中位 **15%**, 大 handoff(>60 轮)中位 **31%**
>   ⇒ 一次改动后的 handoff 若碰巧规模小, 会因为**错误的原因**显得达标
