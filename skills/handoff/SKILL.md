...
>
> ✅ **当前做法(过渡期, 永久设计待定)**: 判据照跑、**结论照写**, 但**接班 prompt 里不下删除指令** ——
> 改成「⚠ 前任 worktree `<路径>`: 我在 `<时刻>` 查过四条判据全过, **但该判据有已知的时间窗口,
> 不要据此删除**; 如需回收请在**远离交接时刻**的时候另行判断」。
> ⚠ **写明时刻**, 因为那是快照不是结论。
>
...
♻️ 前任 worktree: <绝对路径>(分支 <branch>)
   ...
   ⛔ **两步都过了也【不要删】**(2026-08-30 起):四条判据全过而答案是「不能删」的实例已有三个,
      成因是回收检查与新棒搬入落在同一个几分钟窗口里 —— 加判据解决不了。
      ⇒ 把结论如实写给我(路径 + 你查的时刻 + 四条都过),**删这个动作留到远离交接时刻再做**。
