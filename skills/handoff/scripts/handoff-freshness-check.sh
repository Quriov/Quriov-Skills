...
#
# ⚠ 2026-09-14 改: 文件头自称生成物的候选一律跳过并出声(见下方 GEN_MARK 注释)。
#
# Usage:
...
#    ⚠ 2026-08-22 再修 (某仓实撞): 只要求"同一行里有标记词 + 路径"仍然不够 ——
...
# ⚠ 2026-09-14 加(G 线 v2.7 跑 handoff 实撞): 【文件头自称生成物】的候选一律跳过。
#    某仓的声明行 09-02 随 AGENTS.md 拆薄被删 ⇒ 闸门退到下面的常见路径, 第一个命中的 context/active-tracks.md
#    09-10 已改成生成物(文件头「⛔ 整张表是生成物,一个字都别手改」), 而闸门的提示会让人「改一下再跑」——
#    与 08-22 那次同形状: 一个把人推向手改生成物的闸门。
#    判据是结构性的、与仓无关: 要人刷新的正本, 不可能是一份自称「别手改」的文件。只看前 30 行(正文顺口提到不算)。
#    ⛔ 刻意不写死「某仓优先认 worklines/<track>.md」: 那是把一个仓的形态焊进公开脚本(同上方 bot 名那条)。
GEN_MARK='(生成物|别手改|不要手改|勿手改|do not edit|auto-?generated|machine-generated|generated by)'
is_generated() {  # $1 = 绝对路径; 目录不算。不用 grep -q: pipefail 下 head 可能吃 SIGPIPE, 让命中被判成没命中
  [[ -f "$1" ]] && head -n 30 "$1" 2>/dev/null | grep -iE "$GEN_MARK" >/dev/null
}
pick_path() {  # stdin: 候选路径若干; stdout: 第一个真实存在、且不是生成物的
  sed 's#[.,;:)`]*$##' | while read -r c; do
    [[ -e "$ROOT/$c" ]] || continue
    is_generated "$ROOT/$c" && continue
    echo "$c"; break
  done
}
...
if [[ -z "$SOT" ]]; then
  for _c in .claude/active-tracks.yaml context/active-tracks.md docs/active-tracks.md; do
    [[ -f "$ROOT/$_c" ]] || continue
    is_generated "$ROOT/$_c" && continue
    SOT="$ROOT/$_c"; [[ "$_c" == *.yaml ]] && KIND=yaml || KIND=md; break
  done
  if [[ -z "$SOT" && -d "$ROOT/context/worklines" ]]; then SOT="$ROOT/context/worklines"; KIND=dir; fi
fi

# ── 跳过了谁, 必须说出来(「读不到就跳过」的分支不许静默) ──
SKIPPED_GEN=$( { for decl in "$ROOT/CLAUDE.md" "$ROOT/AGENTS.md"; do
                   [[ -f "$decl" ]] && grep -iE "$MARK" "$decl" 2>/dev/null | grep -oE "$PATHRX"
                 done
                 printf '%s\n' .claude/active-tracks.yaml context/active-tracks.md docs/active-tracks.md
               } | sed 's#[.,;:)`]*$##' | sort -u | while read -r c; do is_generated "$ROOT/$c" && echo "$c"; done )
if [[ -n "$SKIPPED_GEN" ]]; then
  echo "ℹ️ handoff-freshness: 跳过了文件头自称生成物的候选 —— 它们不是要人刷新的正本, ⛔ 别手改它们来过闸:"
  printf '%s\n' "$SKIPPED_GEN" | sed 's/^/     · /'
fi
...
#
# ⚠ 2026-08-28 修 (D 线实测报回): 原版只有两层, 第二层是「该文件今天有 commit」——
...
$(git -C "$ROOT" log --since=midnight --format='     · %an — %s' -3 -- "$TARGET" 2>/dev/null)
   → 上面若是自动化任务/别的 session, 说明**本闸没能证明你改过它**;
     **请自己确认 Step 3b 真的刷新了它**: 内容确实没反映本棒结果才去改(改完会走到强判定);
     ⛔ 别为了过闸做无意义的改动 —— 那会让这道闸从此失明。
