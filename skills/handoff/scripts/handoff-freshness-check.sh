...
⚠️ handoff-freshness: ${TARGET#$ROOT/} 今天有 commit, 但**查不出是不是本 session 改的**
   工作区没有它的改动, 本分支相对主干也没有 —— 只剩"今天有人动过它"这一条弱证据。
   今天动过它的是这些(最近 3 条):
$(git -C "$ROOT" log --since=midnight --format='     · %an — %s' -3 -- "$TARGET" 2>/dev/null)
   → 上面若是自动化任务/别的 session, 说明**本闸没能证明你改过它**;
     **请自己确认 Step 3b 真的刷新了它**, 拿不准就改一下再跑(改完会走到强判定)。
   (不阻塞: 也可能是你今天早些时候已提交并合入主干了)
MSG
      exit 0
    fi
