...
# ⚠ 2026-09-14 加(设计系统 v2.7 跑 handoff 实撞): 【文件头自称生成物】的候选一律跳过。
#    眼镜仓的声明行 09-02 随 AGENTS.md 拆薄被删 ⇒ 闸门退到下面的常见路径, 第一个命中的 context/active-tracks.md
...
#    ⛔ 刻意不写死「眼镜仓优先认 worklines/<track>.md」: 那是把一个仓的形态焊进公开脚本(同上方 bot 名那条)。
