...
**三种情形分流** (2026-08-21 用户拍板改的; 此前三种一律 STOP+ASK):
...
⚠ **放松的是"发现之后要不要问", 不是"要不要查"** —— 本步仍是每次必跑的**步骤**。它 2026-08-20/21 两天内在两条线上各救过一次; 某线原话:「我当时并不觉得自己站在死分支上……**如果它是一句『记得检查一下』而不是一个步骤, 我 100% 会跳过**。」把它降级成提醒 = 这道防线消失。
...
**真实事故 (2026-08-05, 某仓 #1201 / 复盘 #1202)**:
...
卡顶没写 → 下一棒只扫卡顶 + pending, handoff 正文里的拍板对它不可见
        → 再下一棒的个人 memory 把「已拍」记成「仍等用户拍」(方向反了)
...
**真实事故 (2026-08-04 确认, 两次现形)**: 某仓 `AGENTS.md` 协议**强制**「Session 结束更新 active-tracks 自己那行」, 而它的 handoff 自动合并判定器白名单只有 `context/handoffs/` 一条 → **每份合规交接 PR 都含这两个文件 → 一律判 block**, 全靠人肉合 (实测 #1203 于 2026-07-31、#1204 于 2026-08-04, 均由人工 merge)。
...
|---|---|---|---|
| B 线 v1.5 | `…7f6553-20` | `…7f6553-12` | **没有**(标题一字未变) |
| G 线 v2.4 | `…1101ff-39` | `…1101ff-c6` | **没有**(标题一字未变) |
| E 线 v2.1→v2.2 | `track-e-v2-0-928166-27` | `memory-framework-analysis-c0e87b-a4` | **换了**, 且**整条线搬了工作目录** ⇒ 连前缀都没了 |
...
同一台机器上的三个反例:
- G 线 v2.4 的工作目录叫 `track-d-v5-1101ff` —— 看着像 D 线 v5, 而 D 线 v5.5 在 `track-d-v5-5-ae58b1`;
- ⭐⭐ **同一个工作目录会在【不同的线】之间转手, 所以一份【当时正确】的记录也会变成误导**:
  目录 `memory-framework-analysis-…` 的分支历史(`git reflog` 实证)是
  `memory-framework-analysis-…` → **`track-a-v4-2-…`**(A 线)→ **`track-e-v2-2-…`**(记忆线)。
  ⚠ **本节初版把它写成「上一棒记错了」—— 那是错的, 已订正**: 上一棒写下它时它**确实**是 A 线的。
  🔑 **所以问题不是"有人记错了", 而是「记对了也会过期」, 而且过期的方向是【看起来仍然合理】** ——
  你看到 `memory-framework-analysis` 会以为那是记忆线, 而它当时是 A 线。**这比"名字不携带身份"更能挡住人的直觉。**
- D 线 v5.4 的地址前缀是 `track-a-v37-handoff-0a5b55`。
...
>
> 📌 真实事故: 某仓协议**强制**同轮更新 `active-tracks.md`, 而它的 handoff 自动合并白名单只认 `context/handoffs/` → **每份合规交接 PR 都被判 block**, 全靠人手动合 (实测 #1203 / #1204)。根因就是本 skill 无条件打包、没有"这个文件本可直推"的概念。
...
SKILL.md 的第 0 条判据与查占用那两节只留动作与判据。下面是被移出来的测量与叙述。
...
> 📌 **两条线各自实测(2026-08-29), 对同一个确实有人住的目录**:
> ```
> lsof +D <路径>                →  15 个进程      ← 真相
> lsof -a -d cwd -c claude      →   2 个 claude   ← 真相
> pgrep -f <路径>               →   0            ← 假阴性
> 那个进程的 cmdline 含此路径吗  →   0            ← 根因
> ```

> 📌 **实测(控制变量, 2026-08-28)**, 对同一个**空目录**:
> ```
> 从外面查                    → 占用进程 0
> 先 cd 进去、再在里面查      → 占用进程 6   ← 全是自己造的
>    zsh(那个 cd) + 查询命令自己 + 管道另一端的几个进程
> ```
